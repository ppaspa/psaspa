...
static inline int
static_is_bench_proto(struct static_proto *p)
{
  return !strcmp(p->p.name, "test_routes");
}

static inline s64
static_timeval_to_us(const struct timeval *tv)
{
  return ((s64) tv->tv_sec * 1000000) + tv->tv_usec;
}

static void
static_emit_bench_and_exit(struct static_proto *p)
{
  struct rusage ru;
  memset(&ru, 0, sizeof(ru));
  getrusage(RUSAGE_SELF, &ru);

  long long wall_us = current_time_now() - p->bench_start_us;
  unsigned long long total_ns = aspa_bench_total_ns;
  long long total_us = total_ns / 1000;
  long long calls = aspa_bench_calls;
  unsigned long long avg_ns = calls ? (total_ns / calls) : 0;
  long long user_us = static_timeval_to_us(&ru.ru_utime);
  long long sys_us = static_timeval_to_us(&ru.ru_stime);

  fprintf(stderr,
          "BENCH proto=%s wall_us=%lld total_ns=%llu total_us=%lld calls=%lld avg_ns=%llu rss_kb=%ld user_us=%lld sys_us=%lld minflt=%ld majflt=%ld nvcsw=%ld nivcsw=%ld\n",
          p->p.name, wall_us, total_ns, total_us, calls, avg_ns, ru.ru_maxrss, user_us, sys_us,
...
static adata *
static_build_aspa_data(struct static_config *cf, struct static_route *r)
{
  uint providers_len = r->aspa_providers ? r->aspa_providers->length : 0;
  adata *a = mb_alloc(cf->c.global->pool, sizeof(adata) + sizeof(struct aspa_payload) + providers_len);
  a->length = sizeof(struct aspa_payload) + providers_len;

  struct aspa_payload *p = (struct aspa_payload *) a->data;
  p->reserved = 0;
  p->prefix = IPA_NONE;

  if (r->aspa_prefix)
  {
    p->net_type = r->aspa_prefix->type;
    p->pxlen = r->aspa_prefix->pxlen;
    p->prefix = net_prefix(r->aspa_prefix);
  }
  else
  {
    p->net_type = 0;
  ...
  if (providers_len)
    memcpy(a->data + sizeof(struct aspa_payload), r->aspa_providers->data, providers_len);
...
static_announce_rte(struct static_proto *p, struct static_route *r)
{
  ...
  if (r->net->type == NET_ASPA)
  {
    ...
    if (!r->aspa_providers)
    {
      log(L_WARN "%s: ASPA %u configured with no provider list, ignoring",
          p->p.name, ((struct net_addr_aspa *) r->net)->asn);
      goto withdraw;
    }

    if (!r->aspa_data)
      r->aspa_data = static_build_aspa_data(cf, r);
...
static_announce_marked(void *P)
{
  ...
  if (static_is_bench_proto(p) && !p->bench_reported)
  {
    p->bench_reported = 1;
    static_emit_bench_and_exit(p);
  }
}
...
static_same_dest(struct static_route *x, struct static_route *y)
{
  ...
  {
    if (!!x->aspa_prefix != !!y->aspa_prefix)
      return 0;

    if (x->aspa_prefix && !net_equal(x->aspa_prefix, y->aspa_prefix))
      return 0;

    return adata_same(x->aspa_providers, y->aspa_providers);
  }
...
static int
static_aspa_rte_better(rte *new, rte *old)
{
  eattr *nea = ea_find(new->attrs->eattrs, EA_ASPA_PROVIDERS);
  eattr *oea = ea_find(old->attrs->eattrs, EA_ASPA_PROVIDERS);

  if (!nea || !oea)
    return !!nea;

  const adata *na = nea->u.ptr;
  const adata *oa = oea->u.ptr;
  int np = aspa_payload_const(na)->net_type != 0;
  int op = aspa_payload_const(oa)->net_type != 0;
  ...
  if (np && op)
  {
    const struct aspa_payload *nap = aspa_payload_const(na);
    const struct aspa_payload *oap = aspa_payload_const(oa);

    if ((nap->net_type == oap->net_type) && (nap->pxlen != oap->pxlen))
      return nap->pxlen > oap->pxlen;
  }
...
{
  if (new->net->n.addr->type == NET_ASPA)
    return static_aspa_rte_better(new, old);
...
static_start(struct proto *P)
{
  ...
  if (static_is_bench_proto(p))
  {
    p->bench_route_count = 0;
    WALK_LIST(r, cf->routes)
      p->bench_route_count++;

    aspa_bench_reset_stats();
    p->bench_start_us = current_time_now();
  }

...
/* Relational operators */
  ...
  INST(FI_ASPA_CHECK_EXPLICIT, 2, 1) {	/* ASPA Check */
    ...
    RTC(3);
    struct rtable *table = rtc->table;
    const net_addr *route_net = (*fs->rte)->net->n.addr;
    enum aspa_result outcome;
    u64 t0 = aspa_bench_now_ns();
    ...
    outcome = aspa_check(table, route_net, v1.val.ad, v2.val.i);
    aspa_bench_total_ns += aspa_bench_now_ns() - t0;
    aspa_bench_calls++;
    RESULT(T_ENUM_ASPA, i, [[ outcome ]]);
  }

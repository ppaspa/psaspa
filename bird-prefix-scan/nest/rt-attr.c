...
static int
aspa_set_format(const struct adata *ad, byte *buf, uint size)
{
  const struct aspa_payload *p = aspa_payload_const(ad);
  const u32 *providers = aspa_payload_providers_const(ad);
  uint provider_count = aspa_payload_provider_count(ad);
  byte *pos = buf;
  byte *end = buf + size;
  ...
  if (!p->net_type)
    pos += bsprintf(pos, "global");
  else
    pos += bsprintf(pos, "%I/%u", p->prefix, p->pxlen);
  ...
  for (uint i = 0; i < provider_count; i++)
  {
    ...
    pos += bsprintf(pos, "%u", providers[i]);
  }
...
get_generic_attr(const eattr *a, byte **buf, int buflen)
{
  ...
  case EA_ASPA_PROVIDERS:
    ...
    *(*buf)++ = ':';
    *(*buf)++ = ' ';
    return aspa_set_format(a->u.ptr, *buf, end - *buf);

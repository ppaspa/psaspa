 ...
 */
u64 aspa_bench_total_ns;
u64 aspa_bench_calls;
void
aspa_bench_reset_stats(void)
{
  aspa_bench_total_ns = 0;
  aspa_bench_calls = 0;
}

static inline void aspa_cache_mark_trie_inactive(struct aspa_cache *cache);

static void
aspa_cache_free(net *n)
{
  if (!n || !n->aspa_cache)
    return;

  aspa_cache_mark_trie_inactive(n->aspa_cache);
  rfree(n->aspa_cache->pool);
  n->aspa_cache = NULL;
}

static inline int
aspa_cache_has_rules(const struct aspa_cache *cache)
{
  return cache && cache->has_any_rule;
}

enum {
  ASPA_PREFIX_FIB_HASH_ORDER = 4,
  /*
   * Hybrid v2 keeps the original scan-first idea, but promotion now depends on
   * recent activity instead of a monotonically increasing lookup counter.
   * This avoids slowly turning every active origin into a trie during large
   * sweeps. When the global budget is full, the weakest active trie may be
   * demoted to make room for a hotter origin.
   */
  ASPA_HYBRID_MIN_PREFIX_RULES = 16,
  ASPA_HYBRID_PROMOTION_SCORE = 2048,
  ASPA_HYBRID_REPLACE_MARGIN = 256,
  ASPA_HYBRID_STALE_SCORE = 256,
  ASPA_HYBRID_DECAY_INTERVAL = 32,
  ASPA_HYBRID_STALE_IDLE_TICKS = 4096,
  ASPA_HYBRID_MAX_RECENT_LOOKUPS = 1024,
  ASPA_HYBRID_MAX_ACTIVE_TRIES = 256,
};

static void aspa_cache_prepare_prefix(struct aspa_cache *cache, net *n);
static inline void aspa_cache_rebuild(rtable *tab, net *n, int need_prefix);

static inline int
aspa_cache_score_from_recent(const struct aspa_cache *cache)
{
  if (!cache || !cache->prefix_rule_count || !cache->prefix_recent_lookups)
    return 0;

  u64 score = (u64) cache->prefix_rule_count * (u64) cache->prefix_recent_lookups;
  return (score > 0xffffffffULL) ? 0xffffffffU : (u32) score;
}

static inline void
aspa_cache_refresh_recent(struct aspa_cache *cache, u32 now_tick)
{
  if (!cache)
    return;

  if (!cache->prefix_last_lookup_tick)
  {
    cache->prefix_last_lookup_tick = now_tick;
    cache->prefix_recent_score = aspa_cache_score_from_recent(cache);
    return;
  }

  u32 elapsed = now_tick - cache->prefix_last_lookup_tick;
  if (elapsed >= ASPA_HYBRID_DECAY_INTERVAL && cache->prefix_recent_lookups)
  {
    u32 decay = elapsed / ASPA_HYBRID_DECAY_INTERVAL;
    cache->prefix_recent_lookups =
      (decay >= cache->prefix_recent_lookups) ? 0 : (cache->prefix_recent_lookups - decay);
  }
...
static inline u32
aspa_cache_touch_lookup(struct aspa_cache *cache)
{
  if (!cache || !cache->owner_tab)
    return 0;

  u32 now_tick = ++cache->owner_tab->aspa_hybrid_lookup_tick;
  aspa_cache_refresh_recent(cache, now_tick);

  if (cache->prefix_recent_lookups < ASPA_HYBRID_MAX_RECENT_LOOKUPS)
    cache->prefix_recent_lookups++;

  cache->prefix_last_lookup_tick = now_tick;
  cache->prefix_recent_score = aspa_cache_score_from_recent(cache);
  return cache->prefix_recent_score;
}

static inline int
aspa_cache_should_promote(const struct aspa_cache *cache)
{
  if (!cache || cache->trie_enabled || cache->trie_accounted)
    return 0;

  if (cache->prefix_rule_count < ASPA_HYBRID_MIN_PREFIX_RULES)
    return 0;

  return cache->prefix_recent_score >= ASPA_HYBRID_PROMOTION_SCORE;
}

static inline int
aspa_table_can_promote(const rtable *tab)
{
  return tab && (tab->aspa_hybrid_trie_count < ASPA_HYBRID_MAX_ACTIVE_TRIES);
}

static inline int
aspa_cache_is_stale(const struct aspa_cache *cache, u32 now_tick)
{
  if (!cache)
    return 1;

  if (cache->prefix_recent_score <= ASPA_HYBRID_STALE_SCORE)
    return 1;

  return cache->prefix_last_lookup_tick &&
    ((now_tick - cache->prefix_last_lookup_tick) >= ASPA_HYBRID_STALE_IDLE_TICKS);
}

static inline void
aspa_cache_mark_trie_active(struct aspa_cache *cache)
{
  if (!cache || cache->trie_accounted || !cache->owner_tab || !cache->prefix_rule_count)
    return;

  cache->trie_accounted = 1;
  cache->owner_tab->aspa_hybrid_trie_count++;
  add_tail(&cache->owner_tab->aspa_hybrid_active_tries, &cache->trie_node);
}

static inline void
aspa_cache_mark_trie_inactive(struct aspa_cache *cache)
{
  if (!cache || !cache->trie_accounted || !cache->owner_tab)
    return;

  if (cache->owner_tab->aspa_hybrid_trie_count)
    cache->owner_tab->aspa_hybrid_trie_count--;

  if (cache->trie_node.next)
    rem_node(&cache->trie_node);

  cache->trie_accounted = 0;
}

static struct aspa_cache *
aspa_table_weakest_active_cache(rtable *tab)
{
  if (!tab || EMPTY_LIST(tab->aspa_hybrid_active_tries))
    return NULL;

  struct aspa_cache *weakest = NULL;
  struct aspa_cache *cache;
  node *nn;
  u32 now_tick = tab->aspa_hybrid_lookup_tick;

  WALK_LIST2(cache, nn, tab->aspa_hybrid_active_tries, trie_node)
  {
    aspa_cache_refresh_recent(cache, now_tick);

    if (!weakest || (cache->prefix_recent_score < weakest->prefix_recent_score) ||
        ((cache->prefix_recent_score == weakest->prefix_recent_score) &&
         (cache->prefix_last_lookup_tick < weakest->prefix_last_lookup_tick)))
      weakest = cache;
  }
...
static inline void
aspa_cache_demote(struct aspa_cache *cache)
{
  if (!cache || !cache->owner_tab || !cache->owner_net || !cache->trie_accounted)
    return;

  aspa_cache_rebuild(cache->owner_tab, cache->owner_net, 0);
}

static int
aspa_cache_try_promote(struct aspa_cache *cache, net *n)
{
  if (!aspa_cache_should_promote(cache))
    return 0;

  rtable *tab = cache->owner_tab;
  if (!tab)
    return 0;

  if (!aspa_table_can_promote(tab))
  {
    struct aspa_cache *weakest = aspa_table_weakest_active_cache(tab);
    if (!weakest)
      return 0;

    if (!aspa_cache_is_stale(weakest, tab->aspa_hybrid_lookup_tick) &&
        (cache->prefix_recent_score < (weakest->prefix_recent_score + ASPA_HYBRID_REPLACE_MARGIN)))
      return 0;

    aspa_cache_demote(weakest);
  }

  aspa_cache_prepare_prefix(cache, n);
  return cache->trie_enabled;
}

static const adata *
aspa_find_global_rule(net *n, int *found)
{
  const adata *global_rule = NULL;
  ...
  for (rte *e = n->routes; e; e = e->next)
  {
    ...
    const adata *rule = ea->u.ptr;
    const struct aspa_payload *payload = aspa_payload_const(rule);

    if (!payload->net_type && !global_rule)
      global_rule = rule;
  }
...
static inline int
aspa_rule_has_prefix(const adata *a)
{
  return aspa_payload_const(a)->net_type != 0;
}

static inline uint
aspa_rule_pxlen(const adata *a)
{
  return aspa_payload_const(a)->pxlen;
}

static inline int
aspa_rule_match_net(const adata *a, const net_addr *net)
{
  const struct aspa_payload *p = aspa_payload_const(a);

  if (!net || (p->net_type != net->type))
    return 0;

  if (p->net_type == NET_IP4)
    return ipa_in_px4(net4_prefix(net), ipa_to_ip4(p->prefix), p->pxlen);

  if (p->net_type == NET_IP6)
    return ipa_in_px6(net6_prefix(net), ipa_to_ip6(p->prefix), p->pxlen);

  return 0;
}

static void
aspa_cache_add_prefix_rule(struct aspa_cache *cache, const adata *rule)
{
  if (!cache || !rule)
    return;
  ...
  if (payload->net_type == NET_IP4)
  {
    if (!cache->prefix_ready_v4)
    {
      cache->trie_v4 = f_new_trie(lp_new_default(cache->pool), 0);
      fib_init(&cache->fib_v4, cache->pool, NET_IP4, sizeof(struct aspa_prefix_node),
        OFFSETOF(struct aspa_prefix_node, fn), ASPA_PREFIX_FIB_HASH_ORDER, NULL);
      cache->prefix_ready_v4 = 1;
    }

    net_addr_ip4 px = NET_ADDR_IP4(ipa_to_ip4(payload->prefix), payload->pxlen);
    trie_add_prefix(cache->trie_v4, (net_addr *) &px, payload->pxlen, payload->pxlen);

    struct aspa_prefix_node *node = fib_get(&cache->fib_v4, (net_addr *) &px);
    if (!node->rule)
      node->rule = rule;
  }
  else if (payload->net_type == NET_IP6)
  {
    if (!cache->prefix_ready_v6)
    {
      cache->trie_v6 = f_new_trie(lp_new_default(cache->pool), 0);
      fib_init(&cache->fib_v6, cache->pool, NET_IP6, sizeof(struct aspa_prefix_node),
        OFFSETOF(struct aspa_prefix_node, fn), ASPA_PREFIX_FIB_HASH_ORDER, NULL);
      cache->prefix_ready_v6 = 1;
    }

    net_addr_ip6 px = NET_ADDR_IP6(ipa_to_ip6(payload->prefix), payload->pxlen);
    trie_add_prefix(cache->trie_v6, (net_addr *) &px, payload->pxlen, payload->pxlen);

    struct aspa_prefix_node *node = fib_get(&cache->fib_v6, (net_addr *) &px);
    if (!node->rule)
      node->rule = rule;
  }
}
...
static struct aspa_cache *
aspa_cache_build(rtable *tab, net *n, int need_prefix)
{
  pool *pool = rp_new(tab->rp, "ASPA cache");
  struct aspa_cache *cache = mb_allocz(pool, sizeof(*cache));
  cache->pool = pool;
  cache->owner_tab = tab;
  cache->owner_net = n;
  cache->trie_enabled = !!need_prefix;
  ...
  for (rte *e = n->routes; e; e = e->next)
  {
    ...
    const adata *rule = ea->u.ptr;
    const struct aspa_payload *payload = aspa_payload_const(rule);
    cache->has_any_rule = 1;

    if (!payload->net_type)
    {
      if (!cache->global_rule)
        cache->global_rule = rule;
      continue;
    }

    cache->prefix_rule_count++;

    if (need_prefix)
      aspa_cache_add_prefix_rule(cache, rule);
  }

  cache->prefix_ready = cache->prefix_ready_v4 || cache->prefix_ready_v6;
  cache->prefix_scanned = need_prefix;
  if (!cache->prefix_ready)
    cache->trie_enabled = 0;
  else if (cache->trie_enabled)
    aspa_cache_mark_trie_active(cache);
  n->aspa_cache = cache;
  return cache;
}

static void
aspa_cache_prepare_prefix(struct aspa_cache *cache, net *n)
{
  if (!cache || cache->prefix_scanned)
    return;
  ...
  cache->prefix_ready = cache->prefix_ready_v4 || cache->prefix_ready_v6;
  cache->prefix_scanned = 1;
  cache->trie_enabled = cache->prefix_ready;
  if (cache->trie_enabled)
    aspa_cache_mark_trie_active(cache);
}

static inline struct aspa_cache *
aspa_cache_get(rtable *tab, net *n, int need_prefix)
{
  struct aspa_cache *cache = n->aspa_cache;
  if (!cache)
    cache = aspa_cache_build(tab, n, need_prefix);
...
static inline void
aspa_cache_rebuild(rtable *tab, net *n, int need_prefix)
{
  aspa_cache_free(n);
  aspa_cache_build(tab, n, need_prefix);
}

static const adata *
aspa_cache_lookup_prefix_rule(const struct aspa_cache *cache, const net_addr *route_net)
{
  if (!cache || !route_net || !cache->prefix_ready)
    return NULL;

  switch (route_net->type)
  {
  case NET_IP4:
  {
    if (!cache->prefix_ready_v4 || !cache->trie_v4)
      return NULL;

    net_addr_ip4 key = NET_ADDR_IP4(net4_prefix(route_net), net4_pxlen(route_net));
    net_addr_ip4 match;
    if (!trie_match_longest_ip4(cache->trie_v4, &key, &match, NULL))
      return NULL;

    struct aspa_prefix_node *node = fib_find((struct fib *) &cache->fib_v4, (net_addr *) &match);
    return node ? node->rule : NULL;
  }

  case NET_IP6:
  {
    if (!cache->prefix_ready_v6 || !cache->trie_v6)
      return NULL;

    net_addr_ip6 key = NET_ADDR_IP6(net6_prefix(route_net), net6_pxlen(route_net));
    net_addr_ip6 match;
    if (!trie_match_longest_ip6(cache->trie_v6, &key, &match, NULL))
      return NULL;

    struct aspa_prefix_node *node = fib_find((struct fib *) &cache->fib_v6, (net_addr *) &match);
    return node ? node->rule : NULL;
  }
...
static const adata *
aspa_scan_lookup_prefix_rule(net *n, const net_addr *route_net)
{
  const adata *selected = NULL;
  uint best_pxlen = 0;
  ...
  for (rte *e = (n ? n->routes : NULL); e; e = e->next)
  {
    ...
    const adata *rule = ea->u.ptr;
    if (!aspa_rule_has_prefix(rule))
      continue;

    if (!aspa_rule_match_net(rule, route_net))
      continue;

    if (!selected || (aspa_rule_pxlen(rule) > best_pxlen))
    {
      selected = rule;
      best_pxlen = aspa_rule_pxlen(rule);
    }
  }
...
enum aspa_result
aspa_check(rtable *tab, const net_addr *route_net, const adata *path, bool force_upstream)
{
  ...
  {
    net_addr_aspa nau;
    net_fill_aspa((net_addr *) &nau, asns[ap]);
    n = net_find(tab, (net_addr *) &nau);
    bool use_prefix_rules = (ap == nsz - 1);

    struct aspa_cache *cache = NULL;
    int found = 0;
    bool down = false, up = false;
    const struct adata *selected = NULL;
    const struct adata *final_rule = NULL;

    if (n)
    {
      if (use_prefix_rules)
      {
        cache = aspa_cache_get(tab, n, 0);
        found = aspa_cache_has_rules(cache);

        if (cache && cache->prefix_rule_count)
        {
          if (cache->prefix_lookup_count < ~0U)
            cache->prefix_lookup_count++;

          aspa_cache_touch_lookup(cache);

          if (!cache->trie_enabled)
            aspa_cache_try_promote(cache, n);

          if (cache->trie_enabled)
          {
            selected = aspa_cache_lookup_prefix_rule(cache, route_net);
          }
          else
            selected = aspa_scan_lookup_prefix_rule(n, route_net);
        }

        final_rule = selected ? selected : cache->global_rule;
      }
      else if (n->aspa_cache)
      {
        cache = n->aspa_cache;
        found = aspa_cache_has_rules(cache);
        final_rule = cache->global_rule;
      }
      else
        final_rule = aspa_find_global_rule(n, &found);
    }

    if (final_rule)
    {
      /* Actually found some ASPA */
      const u32 *providers = aspa_payload_providers_const(final_rule);
      uint provider_count = aspa_payload_provider_count(final_rule);

      for (uint i = 0; i < provider_count; i++)
      {
        if ((ap > 0) && (providers[i] == asns[ap-1]))
          up = true;
        if ((ap + 1 < nsz) && (providers[i] == asns[ap+1]))
          down = true;
...
rte_recalculate(struct channel *c, net *net, rte *new, struct rte_src *src)
{
  ...
  int had_aspa_cache = 0;
  int had_aspa_prefix_cache = 0;
  rte *before_old = NULL;
  ...
  if ((table->addr_type == NET_ASPA) && net->aspa_cache)
    {
      had_aspa_cache = 1;
      had_aspa_prefix_cache = net->aspa_cache->prefix_ready;
      aspa_cache_free(net);
    }
 ...
 skip_stats1:
  ...
  if ((table->addr_type == NET_ASPA) && had_aspa_cache && old && new && net->routes)
    aspa_cache_rebuild(table, net, had_aspa_prefix_cache);
...
rt_setup(pool *pp, struct rtable_config *cf)
{
  ...
  init_list(&t->aspa_hybrid_active_tries);

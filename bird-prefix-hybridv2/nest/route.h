...
typedef struct rtable {
  ...
  u32 aspa_hybrid_trie_count;		/* Active promoted ASPA hybrid tries */
  u32 aspa_hybrid_lookup_tick;		/* Logical clock for hybrid recency tracking */
  list aspa_hybrid_active_tries;	/* Promoted ASPA hybrid tries currently active in this table */
...
enum aspa_result aspa_check(rtable *tab, const net_addr *route_net, const struct adata *path, bool force_upstream);
extern u64 aspa_bench_total_ns;
extern u64 aspa_bench_calls;
void aspa_bench_reset_stats(void);

static inline u64
aspa_bench_now_ns(void)
{
  struct timespec ts;
...
static inline struct aspa_payload *
aspa_payload(adata *a)
{ return (struct aspa_payload *) a->data; }

static inline const struct aspa_payload *
aspa_payload_const(const adata *a)
{ return (const struct aspa_payload *) a->data; }

static inline u32 *
aspa_payload_providers(adata *a)
{ return (u32 *) (a->data + sizeof(struct aspa_payload)); }

static inline const u32 *
aspa_payload_providers_const(const adata *a)
{ return (const u32 *) (a->data + sizeof(struct aspa_payload)); }

static inline uint
aspa_payload_provider_count(const adata *a)
{ return (a->length - sizeof(struct aspa_payload)) / sizeof(u32); }

struct aspa_prefix_node {
  const struct adata *rule;
  struct fib_node fn;
};

struct aspa_cache {
  pool *pool;
  struct rtable *owner_tab;
  net *owner_net;
  const struct adata *global_rule;
  byte has_any_rule;
  byte prefix_scanned;
  byte prefix_ready;
  byte prefix_ready_v4;
  byte prefix_ready_v6;
  byte trie_enabled;
  byte trie_accounted;
  node trie_node;
  uint prefix_rule_count;
  uint prefix_lookup_count;
  uint prefix_recent_lookups;
  u32 prefix_last_lookup_tick;
  u32 prefix_recent_score;
  struct f_trie *trie_v4;
  struct f_trie *trie_v6;
  struct fib fib_v4;
  struct fib fib_v6;
};
